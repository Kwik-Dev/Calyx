    ...
    private func sectionHeading(for row: SettingsRow) -> SectionHeading? {
        ...
        case .cockpitAutoApprove:
            return SectionHeading(
                ...
                subtitle: "Applies to agent-initiated pane commands (run, send keys, palette) and to CLI agents' "
                    + "(Claude Code, Codex, Grok) tool-call approval requests. Off = ask every time."
            )
        ...
        case .agentHookApproval:
            return SectionHeading(
                ...
                subtitle: "Routes Claude Code and Codex permission prompts, and all always-approve Grok tool calls, to the Calyx approval banner. Off = agents decide alone."
            )
